...
const POSITIONING = [
  ...
  'HARD EXCLUSION — never write about, and never make the subject: CRM administration, CRM governance, data',
  'quality / dirty data / mandatory fields / deduplication, or raw pipeline & forecast mechanics. Those are',
  'OFF-MESSAGE. The topic is always GTM engineering, not the plumbing underneath it.',
...
const GTM_SUBTHEMES = [
  'GTM × AI — AI agents & copilots in go-to-market, AI-native revenue motions, where AI actually moves revenue vs hype',
  'GTM × RevOps — revenue operations as an ENGINEERING discipline: systems, instrumentation, the RevOps operating model',
  'GTM × Product / PLG — product-led growth, PLG vs sales-led, product signals driving go-to-market, product × GTM alignment',
  'GTM × Data & Experimentation — running go-to-market as experiments (hypothesis + metric), attribution, what to instrument',
  'GTM × Automation & Tech Stack — what to automate first, the modern GTM stack, build vs buy, killing manual glue',
  'GTM × Systems & Motion — designing the go-to-market motion as a repeatable, compounding system, not heroics',
...
export async function generateWeeklyAgenda(
  ...
  const system = [
    ...
    ALTITUDE,
    '',
    'EVERY post is GTM ENGINEERING — each on a DIFFERENT sub-theme above (AI, RevOps, Product/PLG, data &',
    'experimentation, automation & stack, systems & motion, org & talent). Vary the ARCHETYPE across the week',
    '(opinion / hot take, educational framework, prediction, how-to, story) but the SUBJECT is always GTM',
    'engineering. NEVER CRM, data quality, or raw pipeline/forecast mechanics. Every topic must imply a',
